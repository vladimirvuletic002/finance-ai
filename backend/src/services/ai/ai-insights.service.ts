...
class AIInsightsService {
    ...
    static getDisplayRecurringName(tx: {
        ...
        return tx.merchant?.trim() || tx.category?.name || `Category ${tx.categoryId ?? ""}`;
    }
    ...
    static async getInsights(userId: number) {
        ...
        const currentMonthExpenses = Number(currentMonthExpensesAgg._sum.amount?.toString() ?? 0);
        const previousMonthExpenses = Number(previousMonthExpensesAgg._sum.amount?.toString() ?? 0);
        const currentMonthIncome = Number(currentMonthIncomeAgg._sum.amount?.toString() ?? 0);
        ...
        const recurringTransactions = Array.from(recurringMap.values())
            .filter(item => item.monthKeys.size >= 2 && this.hasStableAmount(item.amounts))
            .map(item => {
                const averageAmount =
                    item.amounts.reduce((sum, val) => sum + val, 0) / item.amounts.length;
        ...
        const predictedNextMonthRecurring = Number(
            recurringTransactions.reduce((sum, item) => sum + item.averageAmount, 0).toFixed(2)
        );

        const rawSavingsEstimate = currentMonthIncome - currentMonthExpenses;
        const currentSavingsEstimate = Number(Math.max(rawSavingsEstimate, 0).toFixed(2));

        const goalAmount = Number(activeGoal?.targetAmount?.toString() ?? 1000);
        const goalCurrency = activeGoal?.currency ?? (topSpendingCategory.currency || "EUR");
        ...
        const remainingAmount = Number(Math.max(goalAmount - currentSavingsEstimate, 0).toFixed(2));

        const recommendation = await AIRecommendationService.generateSavingsRecommendation({
