...
/**
 * Fixed-window counter store shared by the rate limiter and the AI usage quota.
 *
 * Both previously kept counters in a per-process `Map`, which over-counts once
 * more than one instance runs (each replica grants the full quota). This
 * abstraction keeps the exact fixed-window semantics but lets the counters live
 * in Redis when configured, so a single limit is enforced across the fleet.
 *
 ...
 * behaviourally identical to the original in-memory implementations.
 */
